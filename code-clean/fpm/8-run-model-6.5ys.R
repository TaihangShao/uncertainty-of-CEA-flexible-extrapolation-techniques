...
flexmodel_6.5ys<-function(ltHaz,ltHaz_out,sd_bc,sd_bc_out,df_cure,pm_ind){
  ...
  for (i in 1:7){
    ...
    dfGOF[MODi,2] <- sum(ltHaz_out$Events*log(ltHaz_out[[MyDists[[i]]]]) - ltHaz_out[[MyDists[[i]]]]*ltHaz_out$AtRisk) + llCons
  ...
  ##summary results
  if(FP1_pow==0){
    modFP1 <- glm (cbind(Events,AtRisk-Events) ~ I(Time^0*lnTime) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
  }else{modFP1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^FP1_pow)+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz_out)}
  
  
  if(FP2_pow_1 == FP2_pow_2 & FP2_pow_2 != 0){
    modFP2<-glm(cbind(Events,AtRisk-Events) ~ I(Time^FP2_pow_1) + I(Time^FP2_pow_2*lnTime) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
  }else if(FP2_pow_1==0 & FP2_pow_2==0){
    modFP2<-glm(cbind(Events,AtRisk-Events) ~ I(Time^0*lnTime) + I(Time^0*lnTime*lnTime)+ offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out) 
  }else if(FP2_pow_2==0 & FP2_pow_1 != 0){
    modFP2<-glm(cbind(Events,AtRisk-Events) ~ I(Time^MyPowers[[1]][i]) + I(Time^0*lnTime)+ offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
  }else{
    modFP2<-glm(cbind(Events,AtRisk-Events) ~ I(Time^FP2_pow_1) + I(Time^FP2_pow_2) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
  }
  ...
  # Hazard estimates
  dfHazEst3[MODi,] <- predict(modFP1, newdata=Newtime2, type="response") # Extrapolated
  ltHaz_out[md[MODi]] <- predict(modFP1, newdata=ltHaz_out, type="response")  # Within-sample
  ...
  # Hazard estimates
  dfHazEst3[MODi,] <- predict(modFP2, newdata=Newtime2, type="response") # Extrapolated
  ltHaz_out[md[MODi]] <- predict(modFP2, newdata=ltHaz_out, type="response")  # Within-sample
  ...
  bc2 <- subset(niv5y, censrec==1)
  myLnL <- array(dim=5)
  myAIC <- array(dim=5)
  for (i in 1:5){
    glmTemp <- gam(cbind(Events,AtRisk-Events) ~ s(Time, bs="cr", k=i+2, fx=TRUE) + offset(log(timedelta)), knots=list(Time=quantile(bc2$recyrs, seq(from=0, to=1, by=1/(1+i))), length=i+2), family=binomial(link=cloglog), data=ltHaz_out)
    myLnL[i] <- (extractAIC(glmTemp)[2] - 2*extractAIC(glmTemp)[1])*(-0.5)
    myAIC[i] <- extractAIC(glmTemp)[2]
  }
  ...
  i<-RCS_k
  glmTemp1 <- gam(cbind(Events,AtRisk-Events) ~ s(Time, bs="cr", k=i+2, fx=TRUE) + offset(log(timedelta)), knots=list(Time=quantile(bc2$recyrs, seq(from=0, to=1, by=1/(1+i))), length=i+2),family=binomial(link=cloglog), data=ltHaz_out)
  ...
  ltHaz_out[md[MODi]] <- predict(glmTemp1, newdata=ltHaz_out, type="response")
  ...
  for (i in 1:3) {
    ...
    ltHaz_out[md[MODi]] <- summary(rpTemp, t=ltHaz_out$Time, type="hazard")[[1]]$est/12
    
    dfGOF[MODi,1] <- md[MODi]
    dfGOF[MODi,2] <- sum(ltHaz_out$Events*log(ltHaz_out[md[MODi]]) - ltHaz_out[[md[MODi]]]*ltHaz_out$AtRisk) + llCons
  ...
  myAIC <- array(dim=10)
  for (i in 1:10){
    glmTemp <- gam(cbind(Events,AtRisk-Events) ~ s(Time, bs="cr", k=i+2, fx=FALSE) + offset(log(timedelta)), knots=list(Time=quantile(bc2$recyrs, seq(from=0, to=1, by=1/(1+i))), length=i+2), family=binomial(link=cloglog), data=ltHaz_out)
    myLnL[i] <- (extractAIC(glmTemp)[2] - 2*extractAIC(glmTemp)[1])*(-0.5)
    myAIC[i] <- extractAIC(glmTemp)[2]
  }
  ...
  i<-GAM_k
  glmTemp1 <- gam(cbind(Events,AtRisk-Events) ~ s(Time, bs="cr", k=i+2, fx=FALSE) + offset(log(timedelta)), knots=list(Time=quantile(bc2$recyrs, seq(from=0, to=1, by=1/(1+i))), length=i+2),family=binomial(link=cloglog), data=ltHaz_out)
  ...
  myAIC <- array(dim=28)
  for (i in 1:28) {
    fit<-try(MyTemp <- flexsurvmixture(survgef ~ 1, dists = c(dists_MM$dist1[i],dists_MM$dist2[i]), control = list(reltol = 1e-8), sr.control= list(reltol = 1e-8)))
    if("try-error" %in% class(fit)) {
      myAIC[i] <- "error"
    }
    else{
  ...
  dfHazEst3[MODi,]<-summary(mixfit, t = Newtime2$Time*12,type="hazard")[[1]]$est
  ltHaz_out[md[MODi]] <- summary(mixfit, t=ltHaz_out$Time*12,type="hazard")[[1]]$est
  surv_MM_ltHaz_out<-summary(mixfit, t=ltHaz_out$Time*12,type="survival")[[1]]$est
  ...
  dfHazEst3[MODi,]<-df_mod_3$Pred/12
  ltHaz_out[md[MODi]] <- df_mod_1$Pred/12
  ...
  #
  dfSurv4<-as.data.frame(matrix(nrow = 240,ncol = 17))
  dfSurv4[,1]<-dfHaz$Newtime2.Time
  for ( i in 2:15 ) {
  ...
  dfSurv4$V16<-surv_MM_df2
  dfSurv4$V17<-mixcure_surv_mod3
  prime1<-c(0,rep(1,16))
  dfSurv4<-rbind(prime1,dfSurv4)
  colnames(dfSurv4) <-c("Time",md)
  ...
  dfSurv5<-as.data.frame(matrix(nrow = 78,ncol = 17))
  dfSurv5[,1]<-dfHaz2$ltHaz_out.Time
  for ( i in 2:15 ) {
    dftemp<-data.frame(dfHaz2$ltHaz_out.Time,dfHaz2[[i]])
    dftemp<-dftemp %>% 
      dplyr::arrange(dftemp[[1]]) %>% 
      dplyr::mutate(cumhaz = cumsum(dftemp[[2]])) %>% 
      dplyr::mutate(survProp = exp(-1*cumhaz))
  ...
  dfSurv5$V16<-surv_MM_ltHaz_out
  dfSurv5$V17<-mixcure_surv_mod1
  prime1<-c(0,rep(1,16))
  dfSurv5<-rbind(prime1,dfSurv5)
  colnames(dfSurv5) <-c("Time",md)
  
  output<-list(dfGOF=dfGOF,ltHaz_out=ltHaz_out,dfHazEst3=dfHazEst3,dfSurv4=dfSurv4,dfSurv5=dfSurv5)
