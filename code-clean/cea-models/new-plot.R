...
icer_6.5_3ys<-
  ggplot()+geom_point(data=ICER_res_new, aes(x = Incremental_QALY, y =Incremental_Cost),size=3,shape=16,colour="seagreen4",fill="bisque1")+
  geom_point(data=ICER_res_new0, aes(x = Incremental_QALY, y =Incremental_Cost),size=3,shape=16,colour="orange",fill="bisque1")+
  geom_point(data=ICER_res_new,aes(y=Incremental_Cost[224] ,x=Incremental_QALY[224]),shape=16,colour="red",size=4)+
  geom_point(data=ICER_res_new0,aes(y=Incremental_Cost[151] ,x=Incremental_QALY[151]),shape=17,colour="red",size=4)+
  scale_fill_continuous()+
  stat_function(fun=wtp_line, geom="line",xlim = c(-0.5,3),size=1,colour="hotpink")+
  coord_cartesian(ylim=c(0,120000),xlim = c(0,2))+
  theme_bw()+
